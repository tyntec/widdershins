...
function safejson(o,replacer,indent) {
  return jsonTrunc(o,replacer,indent,{ depth: 10, visit_cycles: true });
}
...
function schemaToArray(schema,offset,options,data) {
    ...
    walkSchema(schema,{},wsState,function(schema,parent,state){
        ...
        if (schema["x-widdershins-oldRef"]) {
            ...
            entry.safeType = '['+entry.$ref+'](#'  + slugify(data.api.info.title) + "-" +entry.$ref.toLowerCase()+')';
...
function slugify(text) {
    return text.toString().toLowerCase().trim()
        ...
        .replace(/<code>/, '').replace(/<\/code>/, '') //remove code tags
