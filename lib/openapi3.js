        ...
        if (!p.startsWith('x-')) {
            for (var m in source.paths[p]) {
                if ((m !== 'parameters') && (m !== 'summary') && (m !== 'description') && (!m.startsWith('x-'))) {
                    ...
                    let callbacks = method.operation.callbacks;
                    if(callbacks) {
                        for (var cbKey in callbacks) {
                            let callback = callbacks[cbKey];
                            let server = Object.keys(callback)[0];
                            let cbOperations  = callback[server];
                            for (var cbVerb in cbOperations) {
                                let cbMethod = {}
                                cbMethod.operation = cbOperations[cbVerb];
                                cbMethod.verb = cbVerb;
                                cbMethod.path = server;
                                cbMethod.pathParameters = cbOperations.parameters;
                                var cbMethodUniqueName = (cbMethod.operation.operationId ? cbMethod.operation.operationId : cbVerb + '_' + cbKey + '_' + server).split('?')[0].split('/').join('_');
                                if (data.options.tocSummary && cbMethod.operation.summary) {
                                    cbMethodUniqueName = cbMethod.operation.summary;
                                }
                                if(!callback.methods) callback.methods= {};
                                callback.methods[cbMethodUniqueName] = cbMethod;                                
                            }
                        }
                    }
    ...
    for (let param of data.parameters) {
        ...
        if (pSchema && !param.safeType) {
            ...
            if (pSchema["x-widdershins-oldRef"]) {
                ...
                param.safeType = '[' + schemaName + '](#' + common.slugify(data.api.info.title) + "-" + schemaName.toLowerCase() + ')';
            }
            if (param.refName) param.safeType = '[' + param.refName + '](#' + common.slugify(data.api.info.title) + "-" + param.refName.toLowerCase() + ')';
        }
        if (pSchema) {
...
function getResponses(data) {
        ...
        if (!r.startsWith('x-')) {
            ...
            for (let ct in response.content) {
                ...
                if (contentType.schema && contentType.schema["x-widdershins-oldRef"] && contentType.schema["x-widdershins-oldRef"].startsWith('#/components/')) {
                    ...
                    entry.schema = '[' + schemaName + '](#' + common.slugify(data.api.info.title) + "-" + schemaName.toLowerCase() + ')';
                    entry.$ref = true;
                }
                else {
    ...
    console.log(common.slugify(data.api.info.title))
    console.log("------------------------")
    return responses;
